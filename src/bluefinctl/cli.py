...
def devmode(
    ...
    if enable and disable:
        ...
        raise typer.Exit(1)
    if enable:
        from bluefinctl.core.devmode import toggle_devmode, _check_devmode_active
        if _check_devmode_active().active:
    ...
    if disable:
        from bluefinctl.core.devmode import toggle_devmode, _check_devmode_active
        if not _check_devmode_active().active:
...
def changelogs() -> None:
    ...
    content = ""
    # Try version-specific release first
        ...
        if date:
            try:
                with urllib.request.urlopen(
                    ...
                    releases = __import__("json").loads(r.read())
                body = next(
    ...
    if not content:
        ...
        try:
            with urllib.request.urlopen(
                ...
                content = __import__("json").loads(r.read()).get("body", "")
